...
module.exports.ruleAppliesToContext = function(resources) {
            ...
            if (resource.code.coding){
                ...
                codings.forEach((coding) => {
                    if (coding.code && coding.system === 'http://loinc.org') {
                        observationCodes.push(coding.code);
                    }
                });
            }
        }
    ...
    observationCodes.forEach((code) =>{
        if (calciums[code]){
            calcium = code
        } else if (potassiums[code]){
            potassium = code
        } else if (magnesiums[code]){
            magnesium = code
        } else if (creatinines[code]){
            creatinine = code
        } else if (digoxinLevels[code]){
            digoxinLevel = code
        }
    });
