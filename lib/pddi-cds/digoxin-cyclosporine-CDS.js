...
module.exports.ruleAppliesToContext = function(resources) {
    ...
    resources.forEach((resource) => {
        ...
        if (resource.resourceType === 'MedicationOrder' || resource.resourceType === 'MedicationRequest'){
            med = {
               code: null,
               date: null, // TODO Cuis lists use tuples (in the future perhaps expand with dose)
               prefetch: false
            };
            ...
            if (resource.medicationCodeableConcept){
                if(resource.medicationCodeableConcept.coding){
                    ...
                    codings.forEach((coding) => {
                        if (coding.code && coding.system === 'http://www.nlm.nih.gov/research/umls/rxnorm') {
                            medStatementCuis.push(coding.code);
                        }
                    });
                }
            }
    ...
    medStatementCuis.forEach((cui) => {
        if (cyclosporines[cui]){
            cyclosporineOrder = cui
        } else if (digoxins[cui]){
            digoxinOrder = cui
        } else if (diuretics[cui]){
            diureticOrder = cui
    ...
    // TODO below (for around line 180) need to be able to differentiate medStatementCuis when filling dictionaries
    // TODO also need to allow for dates of MedicationOrder and MedicationStatement to be accounted for
...
module.exports.runRule = function(cdsStateInit) {
    ...
    // Case :
    if ( cyclosporines[cdsState.drugPair.cyclosporine] && digoxins[cdsState.drugPair.digoxin] && cdsState.drugPair.digoxinLevel ){
        // card 2
        if ( digoxinLevels[cdsState.drugPair.digoxinLevel.code] && ((cdsState.drugPair.digoxinLevel.unit === 'ng/mL' && cdsState.drugPair.digoxinLevel.value < 0.9) || (cdsState.drugPair.digoxinLevel.unit === 'nmol/L' && cdsState.drugPair.digoxinLevel.value < 1.2)) ){ 
    ...
    } else if ( cyclosporines[cdsState.drugPair.cyclosporine] && digoxins[cdsState.drugPair.digoxin] && cdsState.drugPair.creatinine && cdsState.drugPair.magnesium && cdsState.drugPair.potassium && cdsState.drugPair.calcium ) {
        if (creatinines[cdsState.drugPair.creatinine.code] && magnesiums[cdsState.drugPair.magnesium.code] && potassiums[cdsState.drugPair.potassium.code] && calciums[cdsState.drugPair.calcium.code]) {
            // scenario 1-1 card 3
